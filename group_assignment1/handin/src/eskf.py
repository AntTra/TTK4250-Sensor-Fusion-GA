...
class ESKF():
    ...
    def predict_from_imu(self,
        ...
        x_est_pred_nom = self.model.predict_nom(
            x_est_prev_nom, z_corr, dt)
        x_est_pred_err = self.model.predict_err(
            x_est_prev, z_corr, dt)
    ...
    def inject(self,
        ...
        pos_inj = x_est_nom.pos + dp
        vel_inj = x_est_nom.vel + dv
        ori_inj = x_est_nom.ori @ RotationQuaterion.from_avec(dori_avec)
        accm_bias_inj = x_est_nom.accm_bias + daccm_bias
        gyro_bias_inj = x_est_nom.gyro_bias + dgyro_bias

        x_nom_inj = NominalState(pos_inj, vel_inj, ori_inj,
                                 accm_bias_inj, gyro_bias_inj)

        P_inj = x_est_err.cov.copy()
