...
class ESKF():
    ...
    def update_err_from_gnss(self,
        ...
        innovation = np.asarray(z_gnss) - np.asarray(z_pred)  # TODO
        H = self.sensor.H(x_nom)  # TODO
        P = x_err.cov  # TODO
        R = self.sensor.R  # TODO
        W = P @ H.T @ np.linalg.solve(S, np.eye(3))  # TODO
        x_err_upd = x_err.mean + W @ innovation  # TODO
        I_WH = np.eye(P.shape[0]) - W @ H  # TODO
    ...
    def inject(self,
        ...
        ori_inj = x_est_nom.ori @ RotationQuaterion.from_avec(x_est_err.mean[6:9])
        ...
        G = np.eye(15)
        G[6:9, 6:9] = np.eye(3) - 0.5 * get_cross_matrix(x_est_err.mean[6:9])
